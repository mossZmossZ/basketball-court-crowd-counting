...
class WorkerFasterRCNN(QtCore.QObject):
    ...
    def processImages(self):
        ...
        cursor.execute("SELECT DISTINCT model FROM image_metadata WHERE model IN ('YOLOv5', 'YOLOv8', 'Mask-RCNN') OR model IS NULL")
        ...
        for index, (image_id, date, image_time, image_path) in enumerate(images):
            try:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (date, image_time, people_count, image_path, predict_image_path, 'Faster-RCNN'))
                conn.commit()
